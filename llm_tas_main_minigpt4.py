...
def get_parser():
    ...
    parser.add_argument('--config',
                        default='config/refcoco/refcoco_testA.yaml',
                        type=str,
                        help='config file')
...
if __name__ == "__main__":
    ...
    with open(f"./caption_llm/minigpt4/{ref_args.dataset}_{ref_args.test_split}{temp}_global.json","r") as f:
        mllm_global_sentences = json.load(f)
    
    if is_add_s_l:
        with open(f"./caption_llm/minigpt4/{ref_args.dataset}_{ref_args.test_split}{temp}_local.json","r") as f:
    ...
    for img, param in tbar:
        ...
        for sent_id, sent in zip(param['sents_id'],param['sents']):
            ...
            if is_add_s_l:
                mllm_local_sentence = mllm_local_sentences[str(sent_id.item())]
                mllm_local_token = tokenize([mllm_local_sentence.lower()], truncate=True).to(device)
                with torch.no_grad():
