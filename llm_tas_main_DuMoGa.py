...
if __name__ == "__main__":
    ...
    # else:
    #     temp = ""
    with open(f"./caption_llm/LLaVA-normal-7b/DuMoGa_{args.test_split}_global.json","r") as f:
        mllm_global_sentences = json.load(f)
    
    if is_add_s_l:
        with open(f"./caption_llm/LLaVA-normal-7b/DuMoGa_{args.test_split}_local.json","r") as f:
    ...
    for img, param in tbar:
        ...
        # if str(ref_id.item()) != "0":
        #     continue

        os.makedirs(f"/home/ljc/TAS/visualization_DuMoGa/{ref_id.item()}", exist_ok=True)
        ...
        for sent_id, sent in zip(param['sents_id'],param['sents']):
            ...
            if add_S_n != "none":
                ...
                local_negative_sents = [("a photo of " + str(chunk)) for chunk in doc.noun_chunks if str(chunk) != "A photo" and not are_near_synonyms(str(chunk), local_sent)]
                # print(local_negative_sents)
                try:
                ...
                logits_image_nega = logits_image_nega.max(-1,keepdim=True)[0]
                # print(logits_image_nega)
                # print(Weight_S_n)
            ...
            os.makedirs(f"/home/ljc/TAS/visualization_DuMoGa/{ref_id.item()}/{sent_str}", exist_ok=True)
            cv2.imwrite(filename=f"/home/ljc/TAS/visualization_DuMoGa/{ref_id.item()}/{sent_str}/ori_image.png", img=ori_img)

            show_mask_image(torch.tensor(pred), plt_image, file_name=f"/home/ljc/TAS/visualization_DuMoGa/{ref_id.item()}/{sent_str}/ours.png")
            show_mask_image(torch.tensor(mask), plt_image, file_name=f"/home/ljc/TAS/visualization_DuMoGa/{ref_id.item()}/{sent_str}/gt.png")
            with open(f"/home/ljc/TAS/visualization_DuMoGa/{ref_id.item()}/{sent_str}/sentences.txt", "w+") as f:
                f.write("Surrounding: "+mllm_global_sentence+"\n")
                f.write("Attribute: "+mllm_local_sentence+"\n")
                f.write("Surrounding: "+sent[0]+"\n")
    ...
    f.write(f'\n\n Model: S_l+S_v+Our_S_n(blur_crop+fuse)(ViT-B)'
            f'\nDataset: DuMoGa / {args.test_split}'
