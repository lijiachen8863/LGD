...
def get_parser():
    ...
    parser.add_argument('--config',
                        default='config/refcoco+/refcoco_p_val.yaml',
                        type=str,
                        help='config file')
...
from visualizer import show_mask_image
...
if __name__ == "__main__":
    ...
    ref_args, args = get_parser()
    Weight_S_l = args.Weight_S_l
    Weight_S_n = args.Weight_S_n * -1

    # 加上list达到节约时间的目的
    # weight_s_n_list = [-0.25*i for i in range(9)]
    # weight_list = [(Weight_S_l, s_n) for s_n in weight_s_n_list]
    weight_list = [(0,0), (0, -1), (0.5, 0)]
    # print(weight_list)
    # exit(0)
    # print(weight_list_iou_dict[(0.6,-0.25)])
    # exit(0)
    ...
    if ref_args.dataset == "refcocog":
        weight_s_n_list = [-0.25*i for i in range(13)]
        weight_list = [(Weight_S_l, s_n) for s_n in weight_s_n_list]
        # if Weight_S_n == -0.7:
        #     Weight_S_n = -1.5
    ...
    weight_list_iou_dict = {}
    for weight in weight_list:
        weight_list_iou_dict[weight]={'cum_I':0, 'cum_U':0, 'iou_list':[]}
    

    with open(f"./caption_llm/LLaVA-normal-7b/{ref_args.dataset}_{ref_args.test_split}{temp}_global.json","r") as f:
        mllm_global_sentences = json.load(f)
    
    if is_add_s_l:
        with open(f"./caption_llm/LLaVA-normal-7b/{ref_args.dataset}_{ref_args.test_split}{temp}_local.json","r") as f:
    ...
    for img, param in tbar:
        ...
        for sent_id, sent in zip(param['sents_id'],param['sents']):
            ...
            mllm_global_sentence = mllm_global_sentences[str(sent_id.item())]
            ...
            S_v_logits_per_image = logit_scale * image_features @ text_features.t()
            ...
            s_n_logits_image_nega = logits_image_nega.mean(-1,keepdim=True)
        
            #  Weight_S_n * logits_image_nega #- 0.05 * logits_text_nega
            ...
            s_l_logits_per_image_S_l = logit_scale * image_features @ mllm_local_features.t()
            
            sent = "_".join(sent[0].split(" "))
            os.makedirs(f"/home/ljc/TAS/visualization/{ref_id.item()}/{sent}", exist_ok=True)
            plt_image = Image.open(param['file_name'][0])
            # 这里加上for循环，主要是对逻辑对的便利
            for (weight_s_l, weight_s_n) in weight_list:
                overall_logits_per_image = S_v_logits_per_image + weight_s_l * s_l_logits_per_image_S_l + weight_s_n * s_n_logits_image_nega
                ...
                if weight_s_l==0 and weight_s_n == 0:
                    show_mask_image(torch.tensor(pred), plt_image, file_name=f"/home/ljc/TAS/visualization/{ref_id.item()}/{sent}/ours_none.png")
                elif weight_s_l==0 and weight_s_n != 0:
                    show_mask_image(torch.tensor(pred), plt_image, file_name=f"/home/ljc/TAS/visualization/{ref_id.item()}/{sent}/ours_no_attribute.png")
                elif weight_s_l!=0 and weight_s_n == 0:
                    show_mask_image(torch.tensor(pred), plt_image, file_name=f"/home/ljc/TAS/visualization/{ref_id.item()}/{sent}/ours_no_surround.png")
                else:
                    print(weight_s_l, weight_s_n)
                    exit(0)
                # iou
                ...
                # # iou_list.append(iou)
                # weight_list_iou_dict[(weight_s_l, weight_s_n)]['cum_I']+=np.sum(inter)
                # weight_list_iou_dict[(weight_s_l, weight_s_n)]['cum_U']+=np.sum(union)
                # weight_list_iou_dict[(weight_s_l, weight_s_n)]['iou_list'].append(iou)
    ...
    #     iou_list = np.stack(weight_list_iou_dict[(weight_s_l, weight_s_n)]['iou_list'])
    ...
    #     iou = iou_list.mean()
    #     oiou = weight_list_iou_dict[(weight_s_l, weight_s_n)]['cum_I'] / weight_list_iou_dict[(weight_s_l, weight_s_n)]['cum_U']
    #     prec = {}
    ...
    #     f = open('new_param_result.txt', 'a+')
    #     f.write(f'\n\n Model: S_l+S_v+Our_S_n(blur_crop+fuse)(ViT-B)'
    #             f'\nDataset: {ref_args.dataset} / {ref_args.test_split} / {ref_args.splitby}'
